...
class GetSubsidyView(APIView):
    ...
    def post(self, request):
        ...
        data = {
            'ID': str(dfs['\"ID\"'].array[0]),
            'URL': str(dfs['\"URL\"'].array[0]),
            'SMALL_NAME': str(dfs['\"SMALL_NAME\"'].array[0]),
            'FULL_NAME': str(dfs['\"FULL_NAME\"'].array[0]),
            'NUMBER_NPA': str(dfs['\"NUMBER_NPA\"'].array[0]),
            'DATE_NPA': str(dfs['\"DATE_NPA\"'].array[0]),
            'DESCRIPTION': str(dfs['\"DESCRIPTION\"'].array[0]),
            'PURPOSE': str(dfs['\"PURPOSE\"'].array[0]),
            'OBJECTIVE': str(dfs['\"OBJECTIVE\"'].array[0]),
            'TYPE_MERA': str(dfs['\"TYPE_MERA\"'].array[0]),
            'TYPE_FORMAT_SUPPORT': str(dfs['\"TYPE_FORMAT_SUPPORT\"'].array[0]),
            'SROK_VOZVRATA': str(dfs['\"SROK_VOZVRATA\"'].array[0]),
            'PROCENT_VOZVRATA': str(dfs['\"PROCENT_VOZVRATA\"'].array[0]),
            'GUARANTE_PERIODE': str(dfs['\"GUARANTE_PERIODE\"'].array[0]),
            'GUARANTEE_COST': str(dfs['\"GUARANTEE_COST\"'].array[0]),
            'APPLIANCE_ID': str(dfs['\"APPLIANCE_ID\"'].array[0]),
            'OKVED2': str(dfs['\"OKVED2\"'].array[0]),
            'COMPLEXITY': str(dfs['\"COMPLEXITY\"'].array[0]),
            'AMOUNT_OF_SUPPORT': str(dfs['\"AMOUNT_OF_SUPPORT\"'].array[0]),
            'REGULARITY_SELECT': str(dfs['\"REGULARITY_SELECT\"'].array[0]),
            'PERIOD': str(dfs['\"PERIOD\"'].array[0]),
            'DOGOVOR': str(dfs['\"DOGOVOR\"'].array[0]),
            'GOS_PROGRAM': str(dfs['\"GOS_PROGRAM\"'].array[0]),
            'EVENT': str(dfs['\"EVENT\"'].array[0]),
            'DOP_INFO': str(dfs['\"DOP_INFO\"'].array[0]),
            'IS_NOT_ACTIVE': str(dfs['\"IS_NOT_ACTIVE\"'].array[0]),
            'PRICHINA_NOT_ACT': str(dfs['\"PRICHINA_NOT_ACT\"'].array[0]),
            'REQ_ZAYAVITEL': str(dfs['\"REQ_ZAYAVITEL\"'].array[0]),
            'REQUEST_PROJECT': str(dfs['\"REQUEST_PROJECT\"'].array[0]),
            'IS_SOFINANCE': str(dfs['\"IS_SOFINANCE\"'].array[0]),
            'DOLYA_ISOFINANCE': str(dfs['\"DOLYA_ISOFINANCE\"'].array[0]),
            'BUDGET_PROJECT': str(dfs['\"BUDGET_PROJECT\"'].array[0]),
            'POKAZATEL_RESULT': str(dfs['\"POKAZATEL_RESULT\"'].array[0]),
            'TERRITORIAL_LEVEL': str(dfs['\"TERRITORIAL_LEVEL\"'].array[0]),
            'REGION_ID': str(dfs['\"REGION_ID\"'].array[0]),
            'RESPONS_STRUCTURE': str(dfs['\"RESPONS_STRUCTURE\"'].array[0]),
            'ORG_ID': str(dfs['\"ORG_ID\"'].array[0]),
        }
    ...
    def post(self, request):
        temp_data = {
            ...
            'data': {
                ...
                "dop_tass": json.loads(request.data['dop_tass']),
                "attrs": json.loads(request.data['attrs']),
                "otr": json.loads(request.data['otr']),
        ...
        # data = requests.post('http://188.93.210.132:5000/predict', timeout=10000, json=temp_data).json()
